#!/usr/bin/env python3
"""LAB05-MC boundary expansion sweep.

The first non-leaky LAB05 sweep selected rel_hi=0.60, abs_hi=0.16 and
strength=1.0, all on the aggressive edge of its search grid. This follow-up
expands that edge before treating those values as a local optimum.

Selection remains A+B only. Crop C is evaluated after selection.
"""
import argparse
import csv
import itertools
...
REL_HI = [0.20, 0.30, 0.40, 0.50, 0.60]
ABS_LO = [0.0, 0.0025, 0.005, 0.01]
ABS_HI = [0.06, 0.08, 0.10, 0.12, 0.14, 0.16]
...
def main():
    ...
    boundary = {
        ...
        "abs_lo_at_lower_boundary": sel["abs_lo"] == min(ABS_LO),
        "strength_at_upper_boundary": sel["strength"] == max(STRENGTH),
    }
    res = {
        "protocol": "LAB05_MC_BOUNDARY_EXPANSION_AB_TRAIN_C_HOLDOUT",
