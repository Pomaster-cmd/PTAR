...
OUTDIR = ROOT / "lab_results" / "lab32b_gtx_pack"
OUT = OUTDIR / "ptar_lab32b_compare.cpp"
...
def main() -> None:
    ...
    text = replace_exact(text, OLD_LAB29_SHA, LAB32B_SHA)
    ...
    stale = [OLD_LAB28_SHA, OLD_LAB29_SHA, "LAB28", "lab28", "LAB29", "lab29", "LAB32C", "lab32c"]
    for token in stale:
        if token in text:
    ...
    required = [
        ...
        "lab31g.cso",
        "lab32b.cso",
        "LAB32B_COMPARE_RAW.csv",
        "LAB32B_COMPARE_SUMMARY.json",
        "if(variant==2)",
        "1.0f/(float)kInW",
        "0.5f/(float)kInW",
        "c.inputSize[0]=(float)kInW",
