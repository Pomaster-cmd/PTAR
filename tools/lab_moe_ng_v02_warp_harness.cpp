...
int main(int argc, char** argv)
{
    if (argc != 9)
    {
        std::cerr << "usage: warp_harness input.f32 inW inH outW outH pixel.cso vertex.cso output.f32\n";
        return 2;
    }
    ...
    {
        std::cerr << "input byte size mismatch: got " << inputBytes.size() << ", expected " << expectedInput << "\n";
        return 4;
    }
    ...
    {
        std::cerr << "WARP feature level below 11_0\n";
        return 6;
    }

    ComPtr<IDXGIDevice> dxgiDev;
    ComPtr<IDXGIAdapter> adapter;
    if (SUCCEEDED(dev.As(&dxgiDev)) && SUCCEEDED(dxgiDev->GetAdapter(&adapter)))
    {
        DXGI_ADAPTER_DESC d{};
        if (SUCCEEDED(adapter->GetDesc(&d)))
            std::wcout << L"adapter=" << d.Description << L"\n";
    }
    std::cout << "driver_type=WARP feature_level=0x" << std::hex << static_cast<unsigned>(actual) << std::dec << "\n";
    ...
    inDesc.MipLevels = 1;
    inDesc.ArraySize = 1;
    inDesc.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
    inDesc.SampleDesc.Count = 1;
    inDesc.Usage = D3D11_USAGE_DEFAULT;
    inDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE;

    D3D11_SUBRESOURCE_DATA init{};
    init.pSysMem = inputBytes.data();
    init.SysMemPitch = inW * 4u * sizeof(float);
    ...
    if (!Check(dev->CreateSamplerState(&sampDesc, &sampler), "CreateSamplerState")) return 12;

    Constants constants{};
    constants.inputSize[0] = static_cast<float>(inW);
    constants.inputSize[1] = static_cast<float>(inH);
    constants.outputSize[0] = static_cast<float>(outW);
    constants.outputSize[1] = static_cast<float>(outH);
    ...
    cbInit.pSysMem = &constants;
    ComPtr<ID3D11Buffer> cb;
    if (!Check(dev->CreateBuffer(&cbDesc, &cbInit, &cb), "CreateBuffer(constants)")) return 13;
    ...
    if (!Check(dev->CreateRasterizerState(&rsDesc, &rs), "CreateRasterizerState")) return 16;
    ...
    const float clear[4] = { -99.0f, -99.0f, -99.0f, -99.0f };
    ctx->ClearRenderTargetView(rtv.Get(), clear);
    ctx->Draw(3, 0);

    ID3D11ShaderResourceView* nullSrv[] = { nullptr };
    ctx->PSSetShaderResources(0, 1, nullSrv);
    ctx->CopyResource(staging.Get(), outputTex.Get());

    D3D11_MAPPED_SUBRESOURCE mapped{};
    if (!Check(ctx->Map(staging.Get(), 0, D3D11_MAP_READ, 0, &mapped), "Map(staging)")) return 17;

    const std::size_t rowBytes = static_cast<std::size_t>(outW) * 4u * sizeof(float);
    ...
    if (!WriteAll(outputPath, contiguous.data(), contiguous.size()))
    {
        std::cerr << "failed to write output\n";
        return 18;
    }

    std::cout << "runtime_pass=1 input=" << inW << "x" << inH << " output=" << outW << "x" << outH << " bytes=" << contiguous.size() << "\n";
    return 0;
}
