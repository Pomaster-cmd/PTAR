...
OUT = ROOT / "lab_results" / "lab32b_full"
...
def patch_assignment(text: str, field: str, rhs: str) -> str:
    pattern = re.compile(rf"(constants\.{re.escape(field)}\s*=\s*)([^;]+)(;)", re.MULTILINE)
    patched, count = pattern.subn(rf"\g<1>{rhs}\3", text)
    if count != 1:
...
def main() -> None:
    for kind, source in SOURCES.items():
        ...
        if "axisUVPair" in patched:
            raise SystemExit(f"{kind}: unexpected LAB32C expanded ABI residue")
        (OUT / f"warp_{kind}_shared.cpp").write_text(patched, encoding="utf-8")
        print(f"prepared {kind}: shared LAB31G/LAB32B 16-byte inverse+half-inverse ABI")
