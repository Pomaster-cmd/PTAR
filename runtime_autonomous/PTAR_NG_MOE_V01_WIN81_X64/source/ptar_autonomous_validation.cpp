// PTAR v0.12.0 autonomous validation host
// Runtime path: precompiled DXBC only. No D3DCompiler API.
...
struct TimingPair
{
    ...
    double k185Ms;
    double deltaMs;
    TimingPair() : pairIndex(0), moeFirst(true), moeMs(0), k185Ms(0), deltaMs(0) {}
};
...
static std::string HexLower(const unsigned char* bytes,size_t n)
{
    ...
    {
        out[i*2u]=hex[(bytes[i]>>4)&15u];
        out[i*2u+1u]=hex[bytes[i]&15u];
    }
    return out;
}

static HRESULT Sha256File(const std::wstring& path,std::string& hexDigest)
{
    hexDigest.clear();
    ...
    DWORD objectBytes=0,cb=0,hashBytes=0;
    ...
    st=BCryptGetProperty(
        alg,BCRYPT_OBJECT_LENGTH,(PUCHAR)&objectBytes,sizeof(objectBytes),&cb,0);
    if(st<0) { BCryptCloseAlgorithmProvider(alg,0); return E_FAIL; }

    st=BCryptGetProperty(
        alg,BCRYPT_HASH_LENGTH,(PUCHAR)&hashBytes,sizeof(hashBytes),&cb,0);
    if(st<0 || hashBytes!=32u)
    {
    ...
    st=BCryptCreateHash(
        alg,&hash,&object[0],objectBytes,0,0,0);
    if(st<0)
    {
        BCryptCloseAlgorithmProvider(alg,0);
        return E_FAIL;
    ...
    // BCryptHashData takes ULONG. Feed large files in bounded chunks.
    size_t offset=0;
    while(offset<data.size())
    {
        const size_t remain=data.size()-offset;
        const ULONG chunk=(ULONG)((std::min)(remain,(size_t)0x40000000u));
        st=BCryptHashData(hash,&data[offset],chunk,0);
        if(st<0)
        {
    ...
    hexDigest=HexLower(&digest[0],digest.size());
    return S_OK;
}

static HRESULT VerifyFileSha256(
    ...
    const char* label)
{
    ...
    if(got!=expectedHex)
    {
        std::cerr<<"[FAIL] SHA-256 mismatch for "<<label
                 <<" expected="<<expectedHex<<" got="<<got<<std::endl;
...
public:
    ...
    HRESULT LoadValidatedPrecompiledShaders(
        const std::wstring& vsPath,
        const std::wstring& moePath,
        const std::wstring& k185Path)
    {
        HRESULT hr=VerifyFileSha256(
            vsPath,"6328bbd87aac73b07d6112de593f781b2769381aae65fa9c2bcfa06fdc68585c","ptar_vs.cso");
        if(FAILED(hr)) return hr;

        hr=VerifyFileSha256(
            moePath,"bb531ad6f797d2db4a362c82673067e39b7aa3a8fbb0ea19f786a053556a2279","ptar_moe_ng_v01_sf5_ps.cso");
        if(FAILED(hr)) return hr;

        hr=VerifyFileSha256(
            k185Path,"6bd926e85f21dd08788ff9d189c472800a5e0eb726091158ed1ecde3d06d8c16","ptar_k185_control_ps.cso");
        if(FAILED(hr)) return hr;

        std::vector<unsigned char> v,m,k;
        if(!ReadBinaryFile(vsPath,v) ||
           !ReadBinaryFile(moePath,m) ||
           !ReadBinaryFile(k185Path,k))
            return HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND);

        if(v.size()<4u || m.size()<4u || k.size()<4u ||
           memcmp(&v[0],"DXBC",4)!=0 ||
           memcmp(&m[0],"DXBC",4)!=0 ||
           memcmp(&k[0],"DXBC",4)!=0)
            return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);

        hr=m_device->CreateVertexShader(&v[0],v.size(),0,&m_vs);
        if(FAILED(hr)) return hr;
        hr=m_device->CreatePixelShader(&m[0],m.size(),0,&m_psMoe);
        if(FAILED(hr)) return hr;
        hr=m_device->CreatePixelShader(&k[0],k.size(),0,&m_psK185);
        if(FAILED(hr)) return hr;

        std::cout<<"[PASS] Validated precompiled DXBC loaded. No runtime HLSL compilation."<<std::endl;
        return S_OK;
    }

    HRESULT RenderMoe(const ImageRGBA& input, UINT outW, UINT outH, ImageRGBA& output)
    {
        ...
        SetPipeline(srv.Get(),rtv.Get(),input.width,input.height,outW,outH,m_psMoe.Get());
        m_context->Draw(3,0);
    ...
    HRESULT Benchmark(UINT inW, UINT inH, UINT outW, UINT outH,
                      TimingStats& moe, TimingStats& k185,
                      std::vector<TimingPair>& pairs)
    {
        ...
        return MeasureInterleaved(
            ...
            m_psMoe.Get(),m_psK185.Get(),moe,k185,pairs);
    }
...
private:
    ...
    HRESULT MeasureInterleaved(
        ...
        UINT inW,UINT inH,UINT outW,UINT outH,
        ID3D11PixelShader* moePs,ID3D11PixelShader* k185Ps,
        TimingStats& moe,TimingStats& k185,
        std::vector<TimingPair>& pairs)
    {
        PTARD3D11GpuTimerRing timer;
        ...
        for(UINT i=0;i<PTAR_WARMUP_FRAMES;++i)
        {
            ...
            if((i&1u)==0u)
            {
                hr=TimedShaderDraw(timer,srv,rtv,inW,inH,outW,outH,moePs,a);
                if(FAILED(hr)) return hr;
                hr=TimedShaderDraw(timer,srv,rtv,inW,inH,outW,outH,k185Ps,b);
                if(FAILED(hr)) return hr;
            }
            else
            {
        ...
        std::vector<double> moeSamples,k185Samples;
        moeSamples.reserve(PTAR_TIMING_SAMPLES);
        k185Samples.reserve(PTAR_TIMING_SAMPLES);
        pairs.clear();
        ...
        for(UINT i=0;i<PTAR_TIMING_SAMPLES;++i)
        {
            TimingPair pair;
            pair.pairIndex=i;
            pair.moeFirst=((i&1u)==0u);

            if(pair.moeFirst)
            {
                hr=TimedShaderDraw(timer,srv,rtv,inW,inH,outW,outH,moePs,pair.moeMs);
                if(FAILED(hr)) return hr;
            ...
            pair.deltaMs=pair.moeMs-pair.k185Ms;
            moeSamples.push_back(pair.moeMs);
            k185Samples.push_back(pair.k185Ms);
            pairs.push_back(pair);
        }

        ID3D11ShaderResourceView* nullSrv=0;
        m_context->PSSetShaderResources(0,1,&nullSrv);
        FillTimingStats(moeSamples,moe);
        FillTimingStats(k185Samples,k185);
        return S_OK;
    }
    ...
    ComPtr<ID3D11PixelShader> m_psMoe;
...
static void WriteTimingCsv(const std::wstring& path,const TimingStats& m,const TimingStats& k)
{
    FILE* f=0;
    if(_wfopen_s(&f,path.c_str(),L"wb")!=0 || !f) return;
    fprintf(f,"variant,samples,median_ms,p90_ms,p95_ms,p99_ms,max_ms\r\n");
    fprintf(f,"PTAR_NG_MOE_V01_SF5,%u,%.9f,%.9f,%.9f,%.9f,%.9f\r\n",
            m.validSamples,m.median,m.p90,m.p95,m.p99,m.maximum);
    fprintf(f,"EDGE_NG_V03_K185_CONTROL,%u,%.9f,%.9f,%.9f,%.9f,%.9f\r\n",
...
static void WriteTimingPairsCsv(const std::wstring& path,const std::vector<TimingPair>& pairs)
{
    FILE* f=0;
    if(_wfopen_s(&f,path.c_str(),L"wb")!=0 || !f) return;
    fprintf(f,"pair_index,order,moe_ms,k185_ms,delta_moe_minus_k185_ms\r\n");
    for(size_t i=0;i<pairs.size();++i)
    {
        const TimingPair& p=pairs[i];
        fprintf(f,"%u,%s,%.9f,%.9f,%.9f\r\n",
                p.pairIndex,p.moeFirst?"MOE_FIRST":"K185_FIRST",
                p.moeMs,p.k185Ms,p.deltaMs);
    }
    fclose(f);
}
...
static void ComputePairedDeltaSummary(
    ...
    UINT& moeFaster,UINT& k185Faster,UINT& ties)
{
    std::vector<double> deltas;
    deltas.reserve(pairs.size());
    moeFaster=k185Faster=ties=0u;
    for(size_t i=0;i<pairs.size();++i)
    {
        const double d=pairs[i].deltaMs;
        deltas.push_back(d);
        if(d<0.0) ++moeFaster;
        else if(d>0.0) ++k185Faster;
        else ++ties;
    }
...
static int Usage()
{
    std::wcout << L"Usage: ptar_autonomous_validation.exe "
                  L"--bundle-root <PTAR autonomous bundle> --out <results> "
                  L"[--allow-any-adapter]"
               << std::endl;
    return 2;
}
...
int wmain(int argc,wchar_t** argv)
{
    ...
    const std::wstring shaderDir=JoinPath(bundleRoot,L"shaders");
    const std::wstring vsPath=JoinPath(shaderDir,L"ptar_vs.cso");
    const std::wstring moePath=JoinPath(shaderDir,L"ptar_moe_ng_v01_sf5_ps.cso");
    const std::wstring k185Path=JoinPath(shaderDir,L"ptar_k185_control_ps.cso");

    hr=gpu.LoadValidatedPrecompiledShaders(vsPath,moePath,k185Path);
    if(FAILED(hr)) { std::wcerr<<L"[FAIL] chargement shaders 0x"<<std::hex<<hr<<std::endl;  return 31; }

    const std::wstring inputDir=JoinPath(bundleRoot,L"corpus\\input_lr");
    const std::wstring expectedDir=JoinPath(bundleRoot,L"corpus\\expected_moe_float32");
    ...
    for(size_t ci=0;ci<ids.size();++ci)
    {
        ...
        hr=gpu.RenderMoe(input,expected.width,expected.height,got);
    ...
    hr=gpu.Benchmark(1280,720,1920,1080,tm,tk,timingPairs);
    if(FAILED(hr))
    {
    ...
    if(summary)
    {
        ...
        fprintf(summary,"PTAR-NG MoE v01 SF5 AUTONOMOUS RUNTIME VALIDATION\r\n");
        fprintf(summary,"runtime_shader_mode=PRECOMPILED_DXBC_ONLY\r\n");
        fprintf(summary,"runtime_d3dcompiler_required=NO\r\n");
        fprintf(summary,"runtime_visual_studio_required=NO\r\n");
        fprintf(summary,"runtime_windows_sdk_required=NO\r\n");
        ...
        fprintf(summary,"moe_median_ms=%.9f\r\n",tm.median);
        fprintf(summary,"moe_p95_ms=%.9f\r\n",tm.p95);
        fprintf(summary,"moe_p99_ms=%.9f\r\n",tm.p99);
        fprintf(summary,"k185_control_median_ms=%.9f\r\n",tk.median);
        fprintf(summary,"median_overhead_moe_minus_k185_ms=%.9f\r\n",tm.median-tk.median);
        ...
        fprintf(summary,"paired_moe_faster_count=%u\r\n",pairedMoeFaster);
        ...
        fprintf(summary,"texture_path_target=1_GATHER4_4_SAMPLE_L_0_UAV\r\n");
        fclose(summary);
    }

    std::wcout<<L"[RESULT] MoE parity maxLSB="<<globalMax<<L" failedCases="<<failedCases<<std::endl;
    std::wcout<<L"[TIMING] MoE median="<<tm.median<<L" ms P95="<<tm.p95<<L" P99="<<tm.p99<<std::endl;
    std::wcout<<L"[TIMING] K185 control median="<<tk.median
              <<L" ms overhead="<<(tm.median-tk.median)<<L" ms"<<std::endl;
    std::wcout<<L"[TIMING-PAIRED] meanDelta="<<pairedMeanDelta
              <<L" ms medianDelta="<<pairedMedianDelta
              <<L" ms MOEfaster="<<pairedMoeFaster
              <<L" K185faster="<<pairedK185Faster
              <<L" ties="<<pairedTies<<std::endl;
    std::wcout<<L"[PNG] 42/42 MoE GPU forensic PNG outputs saved and verified non-zero."<<std::endl;
    std::wcout<<L"[AUTONOMOUS-PASS] Precompiled-CSO runtime completed without compiler/SDK dependency.\n[CLEANUP] Validation complete; COM/D3D resources will be released before COM apartment shutdown."<<std::endl;
