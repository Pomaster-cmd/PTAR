...
static bool wait_owner(HWND p,HWND owner,unsigned timeout=3000){DWORD st=GetTickCount();do{if(GetWindow(p,GW_OWNER)==owner)return true;pump(2);}while(GetTickCount()-st<timeout);return false;}
...
static int fail(int rc,const char* what,QueryFn q,HWND g,HWND p,const RegBackup& b){ModeState s{};if(q)query(q,s);std::printf("RC47_OWNER_HOST=FAIL rc=%d what=%s mode=%u visible=%u owner=%p game=%p gle=%lu\n",rc,what,s.borderlessActive,s.presenterVisible,GetWindow(p,GW_OWNER),g,(unsigned long)GetLastError());restore(b);return rc;}
...
int wmain(){
    ...
    // Exact field condition: real P1U46 presenter starts unowned. RC45's older host
    // accidentally passed game as owner and therefore could never reproduce the black client.
    ...
    const HWND originalOwner=GetWindow(presenter,GW_OWNER);
    if(originalOwner!=nullptr)return fail(13,"presenter-not-unowned",nullptr,game,presenter,rb);
    ...
    if(attach(game,presenter,rw,rh,outW,outH)!=0)return fail(16,"attach",q,game,presenter,rb);
    if(!wait_mode(q,false))return fail(17,"windowed-mode",q,game,presenter,rb);
    if(!wait_owned(presenter,game))return fail(18,"windowed-owner-not-attached",q,game,presenter,rb);
    if(!wait_presenter(game,presenter))return fail(19,"windowed-presenter-geometry",q,game,presenter,rb);

    // Reproduce field occlusion: P1U46 repeatedly raises game HWND_TOP. An owned
    // presenter must remain the top hit window over the game client every time.
    ...
    for(unsigned i=0;i<kRaises;++i){
        ...
        RECT c{};if(!client_rect_screen(game,c))return fail(20,"client",q,game,presenter,rb);POINT pt{(c.left+c.right)/2,(c.top+c.bottom)/2};
        if(WindowFromPoint(pt)!=presenter)return fail(21,"presenter-occluded-by-owner",q,game,presenter,rb);
    }

    if(!set_pref(1)||!wait_mode(q,true))return fail(22,"to-borderless",q,game,presenter,rb);
    if(!wait_owner(presenter,originalOwner))return fail(23,"borderless-owner-not-restored",q,game,presenter,rb);
    ...
    if(!set_pref(0)||!wait_mode(q,false))return fail(25,"back-windowed",q,game,presenter,rb);
    if(!wait_owned(presenter,game)||!wait_presenter(game,presenter))return fail(26,"windowed-reentry",q,game,presenter,rb);
    std::printf("RC47_OWNER_HOST=PASS unowned_field_presenter=PASS windowed_owner_attach=PASS hwnd_top_occlusion_guard=PASS raises=%u borderless_owner_restore=PASS windowed_reentry=PASS\n",kRaises);
