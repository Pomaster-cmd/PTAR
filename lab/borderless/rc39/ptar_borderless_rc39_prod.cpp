...
// RC39 keeps the proven RC38 controller and stable attach export for explicit
// post-transition hosts, but AutoStart MUST use the deferred transition path.
// Field RC38 proved that the loader runs before PTAR installs its final game
// WndProc. Immediate stable attachment is therefore overwritten later.
...
static DWORD WINAPI StableWorker(LPVOID){
    ...
    if(!cur){logline("FAIL stable game proc unavailable; fail-open");prod_starting_clear();return 21;}
    g_gameNext=(WNDPROC)cur;
    if(!set_wndproc(g_game,GameProc)){logline("FAIL stable game subclass install");prod_starting_clear();return 22;}
    g_presenterNext=(WNDPROC)get_wndproc(g_presenter);
    if(!g_presenterNext||!set_wndproc(g_presenter,PresenterProc)){
        set_wndproc(g_game,g_gameNext);logline("FAIL stable presenter subclass install");prod_starting_clear();return 23;
    }
    ...
    logfmt("ACTIVE_STABLE game/presenter",(LONG_PTR)((gw<<16)^gh),(LONG_PTR)((pw<<16)^ph),g_iatHooks);
    return 0;
}
...
static int attach_stable(HWND game,HWND presenter,UINT renderW,UINT renderH,UINT outputW,UINT outputH){
    ...
    if(!game||!presenter||!IsWindow(game)||!IsWindow(presenter)||!renderW||!renderH||!outputW||!outputH||renderW>outputW||renderH>outputH){
        logline("FAIL invalid stable attach args");prod_starting_clear();return -10;
    }
    HMONITOR hm=MonitorFromWindow(presenter,MONITOR_DEFAULTTONEAREST);MONITORINFO mi{};mi.cbSize=sizeof(mi);
    if(!hm||!GetMonitorInfoW(hm,&mi)){logline("FAIL stable monitor resolve");prod_starting_clear();return -11;}
    UINT mw=(UINT)(mi.rcMonitor.right-mi.rcMonitor.left),mh=(UINT)(mi.rcMonitor.bottom-mi.rcMonitor.top);
    if(mw!=outputW||mh!=outputH){logfmt("FAIL stable output/monitor mismatch",mw,mh,outputW,outputH);prod_starting_clear();return -12;}
    ...
    if(!g_initialGameProc){logline("FAIL stable initial game proc");prod_starting_clear();return -13;}
    HANDLE th=CreateThread(nullptr,0,StableWorker,nullptr,0,nullptr);
    if(!th){logline("FAIL stable worker create");prod_starting_clear();return -14;}
    CloseHandle(th);logfmt("ATTACH_STABLE_PENDING",renderW,renderH,outputW,outputH);return 0;
}
...
static bool runtime_layout_ok(HMODULE runtime,BYTE*& base,IMAGE_NT_HEADERS64*& nt){
    ...
    BYTE* call=base+0x0000C2FEu;if(call[0]!=0xE8)return false;
    INT32 disp=0;memcpy(&disp,call+1,sizeof(disp));BYTE* target=call+5+disp;
    if(target<base+0x03500000u||target>=base+nt->OptionalHeader.SizeOfImage)return false;
...
extern "C" __declspec(dllexport) int WINAPI PTAR_BorderlessAutoStart(HMODULE runtime){
    ...
    if(!runtime_layout_ok(runtime,base,nt)){logline("FAIL RC39 runtime layout guard; fail-open");return -20;}
    ...
    if(!game||!presenter||!IsWindow(game)||!IsWindow(presenter)){logline("FAIL RC39 runtime windows unavailable; fail-open");return -21;}
    if(!renderW||!renderH||!outputW||!outputH){logfmt("FAIL RC39 runtime dimensions",renderW,renderH,outputW,outputH);return -22;}
    logfmt("AUTO_START_DEFERRED runtime geometry",renderW,renderH,outputW,outputH);
    // Critical RC39 fix: loader executes before P1U46 installs its final WndProc.
    // Use the existing deferred Worker, which snapshots the pre-PTAR WndProc and
    // subclasses only after that WndProc changes. This prevents PTAR from
    // overwriting GameProc after RC39 has claimed logical 1280x720 authority.
    return PTAR_BorderlessAttach(game,presenter,renderW,renderH,outputW,outputH);
}
