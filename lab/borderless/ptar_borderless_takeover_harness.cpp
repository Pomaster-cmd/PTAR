...
static LRESULT CALLBACK GameProc(HWND h,UINT m,WPARAM w,LPARAM l){
    ...
    if(m==WM_PTAR_LOGICAL_MOUSE){g_lastLogicalX=GET_X_LPARAM(l);g_lastLogicalY=GET_Y_LPARAM(l);return 0;}
    return DefWindowProcW(h,m,w,l);
}

static LRESULT CALLBACK PresenterProc(HWND h,UINT m,WPARAM w,LPARAM l){
    if(m==WM_MOUSEACTIVATE){SetForegroundWindow(g_game);SetActiveWindow(g_game);SetFocus(g_game);++g_focusReturns;return MA_NOACTIVATE;}
    switch(m){
    case WM_MOUSEMOVE:
    ...
    case WM_XBUTTONDOWN:case WM_XBUTTONUP:{
        ...
        SendMessageW(g_game,WM_PTAR_LOGICAL_MOUSE,w,MAKELPARAM(logical.x,logical.y));return 0;
    }
...
static bool backbuffer_size(IDXGISwapChain*sc,UINT&w,UINT&h){ID3D11Texture2D*t=nullptr;if(FAILED(sc->GetBuffer(0,__uuidof(ID3D11Texture2D),(void**)&t))||!t)return false;D3D11_TEXTURE2D_DESC d{};t->GetDesc(&d);t->Release();w=d.Width;h=d.Height;return true;}
...
int main(){
    ...
    POINT zero{0,0};HMONITOR hm=MonitorFromPoint(zero,MONITOR_DEFAULTTOPRIMARY);MONITORINFO mi{};mi.cbSize=sizeof(mi);if(!hm||!GetMonitorInfoW(hm,&mi))return 11;
    g_output=mi.rcMonitor;const UINT outW=UINT(g_output.right-g_output.left),outH=UINT(g_output.bottom-g_output.top);if(outW<320||outH<180)return 12;
    ...
    ID3D11Device*dev=nullptr;ID3D11DeviceContext*ctx=nullptr;D3D_FEATURE_LEVEL fl{};D3D_FEATURE_LEVEL req[]={D3D_FEATURE_LEVEL_11_0,D3D_FEATURE_LEVEL_10_1,D3D_FEATURE_LEVEL_10_0};
    ...
    IDXGIDevice*dx=nullptr;IDXGIAdapter*ad=nullptr;IDXGIFactory*f=nullptr;if(FAILED(dev->QueryInterface(__uuidof(IDXGIDevice),(void**)&dx))||FAILED(dx->GetAdapter(&ad))||FAILED(ad->GetParent(__uuidof(IDXGIFactory),(void**)&f)))return fail("dxgi_factory",21);
    IDXGISwapChain*gs=make_swapchain(f,dev,g_game,g_gameW,g_gameH),*os=make_swapchain(f,dev,g_presenter,outW,outH);if(!gs||!os)return fail("swapchains",22);
    ...
    BOOL fs=TRUE;if(FAILED(gs->GetFullscreenState(&fs,nullptr))||fs)return fail("game_fullscreen",24);if(FAILED(os->GetFullscreenState(&fs,nullptr))||fs)return fail("presenter_fullscreen",25);
    ...
    auto add_render=[&](UINT w,UINT h){w=(std::max)(1u,(std::min)(w,outW));h=(std::max)(1u,(std::min)(h,outH));for(const POINT&p:renders)if(UINT(p.x)==w&&UINT(p.y)==h)return;renders.push_back(POINT{LONG(w),LONG(h)});};
    add_render(320,180);add_render(640,360);add_render(800,450);add_render(960,540);add_render(1024,576);add_render(1280,720);add_render(outW/2,outH/2);add_render((outW*3)/4,(outH*3)/4);add_render(outW,outH);

    std::mt19937 rng(0x50544152u);
    unsigned resizeFailures=0,geometryLeaks=0,mouseFailures=0,presentFailures=0,focusFailures=0;
    unsigned long long transitions=0,pointerProbes=0,hostileRequests=0,presenterGeometryChecks=0,contextDrains=0;
    HRESULT firstResizeHr=S_OK,firstGamePresentHr=S_OK,firstOutputPresentHr=S_OK;
    UINT firstResizeW=0,firstResizeH=0;

    for(unsigned i=0;i<20000;++i){
        POINT r=renders[rng()%renders.size()];if(!set_game_geometry(UINT(r.x),UINT(r.y)))return fail("set_game_geometry",30);

        // DXGI requires every reference to the old swapchain buffers to be gone
        // before ResizeBuffers. Mirror the validated PTAR WARP-resize discipline:
        // unbind all OM state, clear device-context references, then flush.
        ctx->OMSetRenderTargets(0,nullptr,nullptr);
        ctx->ClearState();
        ctx->Flush();
        ++contextDrains;
        HRESULT hr=gs->ResizeBuffers(0,g_gameW,g_gameH,DXGI_FORMAT_UNKNOWN,0);
        if(FAILED(hr)){if(resizeFailures==0){firstResizeHr=hr;firstResizeW=g_gameW;firstResizeH=g_gameH;}++resizeFailures;}
        if(!client_is(g_game,g_gameW,g_gameH)||!verify_swap_size(gs,g_gameW,g_gameH))++geometryLeaks;

        SetWindowPos(g_game,nullptr,g_output.left+13,g_output.top+17,LONG(outW),LONG(outH),SWP_NOZORDER|SWP_NOACTIVATE);pump();++hostileRequests;
        ...
        if((i%97)==0){if(!window_is(g_presenter,g_output)||!client_is(g_presenter,outW,outH)||!verify_swap_size(os,outW,outH))++geometryLeaks;++presenterGeometryChecks;}

        Geometry geo=geometry();
        for(unsigned k=0;k<16;++k){POINT logical{LONG(rng()%g_gameW),LONG(rng()%g_gameH)};POINT physical=geo.logical_to_physical(logical);POINT pcpt=physical;ScreenToClient(g_presenter,&pcpt);g_lastLogicalX=g_lastLogicalY=-1;SendMessageW(g_presenter,WM_MOUSEMOVE,0,MAKELPARAM(pcpt.x,pcpt.y));if(g_lastLogicalX<0||g_lastLogicalY<0||std::abs(g_lastLogicalX-logical.x)>1||std::abs(g_lastLogicalY-logical.y)>1)++mouseFailures;++pointerProbes;}
        if((i%211)==0){SetActiveWindow(g_decoy);SetFocus(g_decoy);LRESULT ma=SendMessageW(g_presenter,WM_MOUSEACTIVATE,reinterpret_cast<WPARAM>(g_decoy),MAKELPARAM(HTCLIENT,WM_LBUTTONDOWN));if(ma!=MA_NOACTIVATE||GetActiveWindow()!=g_game||GetFocus()!=g_game)++focusFailures;}

        if((i%5)==0){
            HRESULT gh=gs->Present(0,DXGI_PRESENT_TEST),oh=os->Present(0,DXGI_PRESENT_TEST);
            if(FAILED(gh)||FAILED(oh)){if(presentFailures==0){firstGamePresentHr=gh;firstOutputPresentHr=oh;}++presentFailures;}
        }
        ++transitions;
    }

    if(resizeFailures||geometryLeaks||mouseFailures||presentFailures||focusFailures){
        std::printf("FAIL resize=%u geometry=%u mouse=%u present=%u focus=%u transitions=%llu probes=%llu clamps=%llu drains=%llu first_resize_hr=0x%08lx first_resize=%ux%u first_game_present_hr=0x%08lx first_output_present_hr=0x%08lx\n",resizeFailures,geometryLeaks,mouseFailures,presentFailures,focusFailures,transitions,pointerProbes,g_windowClamps,contextDrains,(unsigned long)firstResizeHr,firstResizeW,firstResizeH,(unsigned long)firstGamePresentHr,(unsigned long)firstOutputPresentHr);return 40;
    }

    std::printf("PASS PTAR_BORDERLESS_TAKEOVER_LAB\n");
    std::printf("transitions=%llu pointer_probes=%llu hostile_native_reassertions=%llu presenter_geometry_checks=%llu window_clamps=%llu focus_returns=%llu context_drains=%llu\n",transitions,pointerProbes,hostileRequests,presenterGeometryChecks,g_windowClamps,g_focusReturns,contextDrains);
    ...
    std::printf("dxgi=both_windowed+NO_WINDOW_CHANGES+NO_ALT_ENTER; ResizeBuffers_precondition=OM_unbound+ClearState+Flush; presenter=owned+NOACTIVATE+TOOLWINDOW+NOT_TRANSPARENT+NOT_TOPMOST\n");

    os->Release();gs->Release();f->Release();ad->Release();dx->Release();ctx->ClearState();ctx->Release();dev->Release();DestroyWindow(g_presenter);DestroyWindow(g_decoy);DestroyWindow(g_game);return 0;
}
