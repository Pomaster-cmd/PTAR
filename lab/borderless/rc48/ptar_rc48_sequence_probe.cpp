...
static void pump(unsigned ms=1){DWORD s=GetTickCount();do{MSG m{};while(PeekMessageW(&m,nullptr,0,0,PM_REMOVE)){TranslateMessage(&m);DispatchMessageW(&m);}Sleep(0);}while(GetTickCount()-s<ms);}
...
static bool wait_mode(QueryFn q,bool b,unsigned timeout,unsigned& elapsed,ModeState& last,unsigned& qfails,unsigned& installed0,unsigned& wrongmode){DWORD st=GetTickCount();qfails=installed0=wrongmode=0;do{ModeState s{};if(!query(q,s)){++qfails;}else{last=s;if(!s.installed)++installed0;else if((s.borderlessActive!=0)==b){elapsed=GetTickCount()-st;return true;}else ++wrongmode;}pump(1);}while(GetTickCount()-st<timeout);elapsed=GetTickCount()-st;return false;}
static int fail(int rc,const char* tag,QueryFn q,HWND g,HWND p,unsigned elapsed=0,unsigned qf=0,unsigned i0=0,unsigned wm=0){ModeState s{};bool qok=query(q,s);RECT gr{},pr{};if(g)GetWindowRect(g,&gr);if(p)GetWindowRect(p,&pr);std::printf("RC48_SEQUENCE=FAIL rc=%d tag=%s qok=%d installed=%u mode=%u pref=%ld elapsed=%u qfails=%u installed0=%u wrongmode=%u gstyle=0x%llx pstyle=0x%llx parent=%p owner=%p grect=%ld,%ld,%ld,%ld prect=%ld,%ld,%ld,%ld\n",rc,tag,qok?1:0,s.installed,s.borderlessActive,s.windowStylePreference,elapsed,qf,i0,wm,(unsigned long long)(g?GetWindowLongPtrW(g,GWL_STYLE):0),(unsigned long long)(p?GetWindowLongPtrW(p,GWL_STYLE):0),p?GetParent(p):nullptr,p?GetWindow(p,GW_OWNER):nullptr,gr.left,gr.top,gr.right,gr.bottom,pr.left,pr.top,pr.right,pr.bottom);std::fflush(stdout);return rc;}
int wmain(){
    ...
    constexpr unsigned kPairs=2500;
    ...
    ModeState before{};query(q,before);std::printf("RC48_SEQUENCE_DIRECT_PASS pairs=%u toB=%llu toW=%llu pref=%ld\n",kPairs,before.transitionsToBorderless,before.transitionsToWindowed,before.windowStylePreference);std::fflush(stdout);
    ...
    std::printf("RC48_SEQUENCE=PASS direct_pairs=%u direct_transitions=%u registry_borderless_ms=%u registry_windowed_ms=%u final_pref=%ld final_mode=%u\n",kPairs,kPairs*2u,regB,regW,s.windowStylePreference,s.borderlessActive);std::fflush(stdout);return 0;
}
