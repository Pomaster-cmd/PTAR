...
int wmain(){
    ...
    DWORD waitStart=GetTickCount();while(!exact_child(game,presenter)&&GetTickCount()-waitStart<5000)pump(2);if(!exact_child(game,presenter))return fail(21,"child-not-established",q,game,presenter,rb);
