...
static void emit_result(const char* line){
    if(line){std::fputs(line,stdout);std::fputc('\n',stdout);std::fflush(stdout);}
    HANDLE h=CreateFileW(L"PREF_STRESS_RESULT.txt",GENERIC_WRITE,FILE_SHARE_READ,nullptr,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,nullptr);
    if(h!=INVALID_HANDLE_VALUE){DWORD wr=0;if(line)WriteFile(h,line,(DWORD)strlen(line),&wr,nullptr);WriteFile(h,"\r\n",2,&wr,nullptr);FlushFileBuffers(h);CloseHandle(h);}
}
template<class T>static void rel(T*&p){if(p){p->Release();p=nullptr;}}
static int fail(int rc,const char* tag,QueryFn q,HWND g,HWND p,const Backup& b,LONG regRc=0){ModeState s{};query(q,s);RECT gr{},pr{};if(g)GetWindowRect(g,&gr);if(p)GetWindowRect(p,&pr);char line[1024]{};sprintf_s(line,"RC48_PREF_STRESS=FAIL rc=%d tag=%s reg_rc=%ld registry=%d installed=%u mode=%u query_pref=%ld gstyle=0x%llx pstyle=0x%llx pex=0x%llx parent=%p owner=%p grect=%ld,%ld,%ld,%ld prect=%ld,%ld,%ld,%ld",rc,tag,regRc,read_pref(),s.installed,s.borderlessActive,s.windowStylePreference,(unsigned long long)(g?GetWindowLongPtrW(g,GWL_STYLE):0),(unsigned long long)(p?GetWindowLongPtrW(p,GWL_STYLE):0),(unsigned long long)(p?GetWindowLongPtrW(p,GWL_EXSTYLE):0),p?GetParent(p):nullptr,p?GetWindow(p,GW_OWNER):nullptr,gr.left,gr.top,gr.right,gr.bottom,pr.left,pr.top,pr.right,pr.bottom);emit_result(line);restore(b);return rc;}
int wmain(){
    DeleteFileW(L"PREF_STRESS_RESULT.txt");
    ...
    char line[512]{};sprintf_s(line,"RC48_PREF_STRESS=PASS pairs=%u transitions=%u max_borderless_ms=%u max_windowed_ms=%u avg_borderless_ms=%llu avg_windowed_ms=%llu swap_output_stable=PASS black_parent_repaint=PASS",kPairs,kPairs*2,maxB,maxW,sumB/kPairs,sumW/kPairs);emit_result(line);
