...
static Pixels sample_rect(const RECT& r,BYTE er,BYTE eg,BYTE eb){Pixels s{};RECT screen{0,0,GetSystemMetrics(SM_CXSCREEN),GetSystemMetrics(SM_CYSCREEN)},vis{};if(!IntersectRect(&vis,&r,&screen))return s;HDC dc=GetDC(nullptr);if(!dc)return s;for(int yy=1;yy<=5;++yy)for(int xx=1;xx<=5;++xx){int x=vis.left+(vis.right-vis.left)*xx/6,y=vis.top+(vis.bottom-vis.top)*yy/6;COLORREF c=GetPixel(dc,x,y);if(c==CLR_INVALID)continue;BYTE rr=GetRValue(c),gg=GetGValue(c),bb=GetBValue(c);++s.n;s.ar+=rr;s.ag+=gg;s.ab+=bb;if(abs((int)rr-er)<=60&&abs((int)gg-eg)<=60&&abs((int)bb-eb)<=60)++s.expected;if(rr<20&&gg<20&&bb<20)++s.black;}ReleaseDC(nullptr,dc);return s;}
...
int WINAPI wWinMain(HINSTANCE inst,HINSTANCE,LPWSTR,int){
    ...
    const UINT outW=(UINT)GetSystemMetrics(SM_CXSCREEN),outH=(UINT)GetSystemMetrics(SM_CYSCREEN);const UINT renderW=(outW*2u)/3u,renderH=(outH*2u)/3u;
    fwprintf(log,L"TRUE_RUNTIME_HOST_BEGIN exe=Warhammer.exe pid=%lu native=%ux%u requested_render=%ux%u\n",GetCurrentProcessId(),outW,outH,renderW,renderH);fflush(log);
    ...
    bool visible=SUCCEEDED(last)&&final.n>=20&&final.expected>=final.n*3/5&&final.black<=final.n/5;fwprintf(log,L"TRUE_P1U46_WINDOWED_VISIBLE_PIXELS=%ls\n",visible?L"PASS":L"FAIL");fflush(log);
