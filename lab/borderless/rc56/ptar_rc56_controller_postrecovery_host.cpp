...
struct ModeState{UINT size,installed,borderlessActive,presenterVisible;ULONG_PTR gameStyle,presenterStyle;unsigned long long transitionsToWindowed,transitionsToBorderless;LONG targetLeft,targetTop,targetRight,targetBottom,windowStylePreference;unsigned long long presenterFollows,gameCoercionClamps,presenterClamps,rejectedWindowSaves;LONG savedLeft,savedTop,savedRight,savedBottom;};
...
// The hosted runner is only 1024x768. Exact P1U46 is nevertheless configured
// for the field contract 1280x720 -> 1920x1080 and its own input-envelope
// recovery therefore changes the synthetic game's physical HWND later in
// startup. RC56 must not be blamed for that P1U46-owned recovery. Wait beyond
// that phase, then freeze the settled game geometry as the invariant for the
// RC56 mode-transition stress.
static bool wait_postrecovery_stable(Driver&d,QueryModeFn q,HWND game,bool wantBorderless,Geometry&base,FILE*f,unsigned timeout=70000){
    const DWORD st=GetTickCount();DWORD installedSince=0;Geometry last{};bool have=false;unsigned stable=0;ModeState ms{};
    do{
        if(!d.frame())return false;ms={};ms.size=sizeof(ms);if(q(&ms)!=0){pump(20);continue;}
        Geometry now{};if(!ms.installed||((ms.borderlessActive!=0)!=wantBorderless)||!geom(game,now)){installedSince=0;stable=0;have=false;pump(20);continue;}
        if(!installedSince)installedSince=GetTickCount();
        if(have&&same_geom(now,last))++stable;else stable=0;last=now;have=true;
        const DWORD installedAge=GetTickCount()-installedSince;
        if(installedAge>=35000&&stable>=12){base=now;log_geom(f,L"GAME_POSTRECOVERY_BASELINE",base);fwprintf(f,L"MODE_POSTRECOVERY installed=%u borderless=%u installedAgeMs=%lu toB=%llu toW=%llu follows=%llu coercion=%llu\n",ms.installed,ms.borderlessActive,(unsigned long)installedAge,ms.transitionsToBorderless,ms.transitionsToWindowed,ms.presenterFollows,ms.gameCoercionClamps);fflush(f);return true;}
        pump(80);
    }while(GetTickCount()-st<timeout);
    fwprintf(f,L"POSTRECOVERY_WAIT_FAIL installed=%u borderless=%u stable=%u installedAgeMs=%lu\n",ms.installed,ms.borderlessActive,stable,installedSince?(unsigned long)(GetTickCount()-installedSince):0ul);fflush(f);return false;
}
...
static bool wait_transition(Driver&d,QueryModeFn q,HWND game,HWND presenter,const Geometry&base,bool borderless,FILE*f,const wchar_t*tag,unsigned timeout=10000){DWORD st=GetTickCount();ModeState ms{};Geometry g{};unsigned deviations=0;do{if(!d.frame())return false;ms={};ms.size=sizeof(ms);q(&ms);if(!geom(game,g))return false;if(!same_geom(g,base))++deviations;if(ms.installed&&((ms.borderlessActive!=0)==borderless)&&same_geom(g,base)&&presenter_matches(presenter,game,borderless)){Geometry p{};geom(presenter,p);fwprintf(f,L"TRANSITION_%ls=PASS borderless=%u game=%ux%u presenter=%ux%u deviations=%u toB=%llu toW=%llu follows=%llu\n",tag,ms.borderlessActive,g.cw,g.ch,p.cw,p.ch,deviations,ms.transitionsToBorderless,ms.transitionsToWindowed,ms.presenterFollows);fflush(f);return true;}pump(20);}while(GetTickCount()-st<timeout);Geometry p{};geom(presenter,p);fwprintf(f,L"TRANSITION_%ls=FAIL wanted=%u actual=%u installed=%u game=%ux%u presenter=%ux%u deviations=%u\n",tag,borderless?1u:0u,ms.borderlessActive,ms.installed,g.cw,g.ch,p.cw,p.ch,deviations);fflush(f);return false;}

int WINAPI wWinMain(HINSTANCE inst,HINSTANCE,LPWSTR cmd,int){
    const bool direct=cmd&&wcsstr(cmd,L"direct");FILE*f=nullptr;_wfopen_s(&f,direct?L"RC56_CONTROLLER_DIRECT.txt":L"RC56_CONTROLLER_STRESS.txt",L"wb");if(!f)return 90;RegBackup rb{};backup_pref(rb);if(!set_pref(direct?1u:0u)){fclose(f);return 10;}
    ...
    Geometry base{};if(!wait_postrecovery_stable(d,q,game,direct,base,f)){restore_pref(rb);fclose(f);return 17;}if(!presenter_matches(presenter,game,direct)){Geometry p{};geom(presenter,p);log_geom(f,L"PRESENTER_BASELINE_MISMATCH",p);restore_pref(rb);fclose(f);return 18;}
    if(direct){fwprintf(f,L"DIRECT_INITIAL=PASS\n");if(!set_pref(0)||!wait_transition(d,q,game,presenter,base,false,f,L"DIRECT_TO_WINDOWED")){restore_pref(rb);fclose(f);return 20;}if(!set_pref(1)||!wait_transition(d,q,game,presenter,base,true,f,L"DIRECT_BACK_BORDERLESS")){restore_pref(rb);fclose(f);return 21;}fwprintf(f,L"RC56_CONTROLLER_DIRECT=PASS game_untouched=1 presenter_only=1 exact_runtime=1 postrecovery=1\n");fflush(f);}else{constexpr unsigned cycles=80;for(unsigned i=0;i<cycles;++i){if(!set_pref(1)||!wait_transition(d,q,game,presenter,base,true,f,L"BORDERLESS")){fwprintf(f,L"FAIL_CYCLE=%u phase=B\n",i);restore_pref(rb);fclose(f);return 30;}if(!set_pref(0)||!wait_transition(d,q,game,presenter,base,false,f,L"WINDOWED")){fwprintf(f,L"FAIL_CYCLE=%u phase=W\n",i);restore_pref(rb);fclose(f);return 31;}}ModeState ms{};ms.size=sizeof(ms);q(&ms);if(ms.borderlessActive||ms.transitionsToBorderless<cycles||ms.transitionsToWindowed<cycles){restore_pref(rb);fclose(f);return 32;}fwprintf(f,L"RC56_CONTROLLER_STRESS=PASS cycles=%u game_untouched=1 presenter_only=1 exact_runtime=1 postrecovery=1 toB=%llu toW=%llu follows=%llu coercion=%llu presenterClamps=%llu\n",cycles,ms.transitionsToBorderless,ms.transitionsToWindowed,ms.presenterFollows,ms.gameCoercionClamps,ms.presenterClamps);fflush(f);}
