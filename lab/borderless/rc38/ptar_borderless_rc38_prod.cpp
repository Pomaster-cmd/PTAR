#include "ptar_borderless_rc38.cpp"
...
extern "C" __declspec(dllexport) int WINAPI PTAR_BorderlessAutoStart(HMODULE runtime){
    ...
    if(!runtime_layout_ok(runtime,base,nt)){logline("FAIL RC38 runtime layout guard; fail-open");return -20;}
    ...
    if(!game||!presenter||!IsWindow(game)||!IsWindow(presenter)){logline("FAIL RC38 runtime windows unavailable; fail-open");return -21;}
    if(!renderW||!renderH||!outputW||!outputH){logfmt("FAIL RC38 runtime dimensions",renderW,renderH,outputW,outputH);return -22;}
    logfmt("AUTO_START runtime geometry",renderW,renderH,outputW,outputH);
    return attach_stable(game,presenter,renderW,renderH,outputW,outputH);
}
