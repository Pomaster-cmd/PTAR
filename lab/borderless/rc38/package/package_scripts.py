...
def _fill(s:str, values:dict[str,str])->str:
    for k,v in values.items(): s=s.replace('@@'+k+'@@',v)
    if '@@' in s: raise ValueError('unexpanded template token')
    return s.replace('\n','\r\n')

def bat_scripts()->dict[str,str]:
    def simple(action,title):
        return (f'@echo off\r\nsetlocal EnableExtensions\r\ncd /d "%~dp0"\r\ntitle {title}\r\n'
                f'"%SystemRoot%\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%~dp0{OVERLAY}\\{action}.ps1"\r\n'
                'set "RC=%ERRORLEVEL%"\r\necho.\r\npause\r\nexit /b %RC%\r\n')
    uninstall=f'''@echo off\r
setlocal EnableExtensions\r
cd /d "%~dp0"\r
title PTAR RC38 BORDERLESS AUTHORITY - DESINSTALLATION COMPLETE SECURISEE\r
for %%I in ("%~dp0.") do set "ROOT=%%~fI"\r
set "TMP=%TEMP%\\PTAR_RC38_UNINSTALL_%RANDOM%_%RANDOM%"\r
mkdir "%TMP%" >nul 2>&1\r
copy /y "%ROOT%\\_PTAR_UNINSTALL\\PTAR_SAFE_UNINSTALL.ps1" "%TMP%\\PTAR_SAFE_UNINSTALL.ps1" >nul || exit /b 20\r
copy /y "%ROOT%\\{OVERLAY}\\uninstall_overlay.ps1" "%TMP%\\uninstall_overlay.ps1" >nul || exit /b 21\r
"%SystemRoot%\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%TMP%\\uninstall_overlay.ps1" -Root "%ROOT%" -Phase Pre -StateFile "%TMP%\\game_root.txt"\r
if errorlevel 1 set "RC=%ERRORLEVEL%"&goto :done\r
"%SystemRoot%\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%TMP%\\PTAR_SAFE_UNINSTALL.ps1" -Root "%ROOT%"\r
if errorlevel 1 set "RC=%ERRORLEVEL%"&goto :done\r
"%SystemRoot%\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%TMP%\\uninstall_overlay.ps1" -Root "%ROOT%" -Phase Post -StateFile "%TMP%\\game_root.txt"\r
set "RC=%ERRORLEVEL%"\r
:done\r
echo.\r
if "%RC%"=="0" (echo [PASS] Desinstallation PTAR + RC38 terminee.) else (echo [FAIL] Desinstallation interrompue - code %RC%.)\r
echo Appuyez sur une touche pour fermer.\r
pause >nul\r
if "%RC%"=="0" (start "" /b "%ComSpec%" /d /c "ping -n 3 127.0.0.1 >nul & del /f /q ^"%ROOT%\\00-DESINSTALLER_BORDERLESS_AUTHORITY_RC38.bat^" >nul 2>&1 & rd /s /q ^"%TMP%^" >nul 2>&1") else (rd /s /q "%TMP%" >nul 2>&1)\r
exit /b %RC%\r
'''
    return {
      '00-INSTALL_BORDERLESS_AUTHORITY_RC38.bat':simple('install','PTAR RC38 BORDERLESS AUTHORITY - INSTALL'),
      '00-VERIFY_BORDERLESS_AUTHORITY_RC38.bat':simple('verify','PTAR RC38 BORDERLESS AUTHORITY - VERIFY'),
      '00-ROLLBACK_BORDERLESS_AUTHORITY_RC38.bat':simple('rollback','PTAR RC38 BORDERLESS AUTHORITY - ROLLBACK GITHUB MAIN'),
      '00-DESINSTALLER_BORDERLESS_AUTHORITY_RC38.bat':uninstall,
    }
...
$ExpectedRuntime='@@RUNTIME@@';$ExpectedIni='@@INI@@';$ExpectedVersion='@@VERSION@@';$ExpectedSidecar='@@SIDECAR@@'
$KnownPtar=@(@@KNOWN@@)
...
$a=Join-Path $g 'd3d11.dll';$c=Join-Path $g 'win81_nis_dx11_x64.dll';$i=Join-Path $g 'win81_nis.ini';$v=Join-Path $g 'win81_nis_version.txt';$s=Join-Path $g 'ptar_borderless.dll'
...
$state=Join-Path $StateRoot ('INSTALL_RC38_'+(Get-Date -Format 'yyyyMMdd_HHmmss_fff'));New-Item -ItemType Directory -Path $state -Force|Out-Null
...
function F([string]$s,[int]$c=1){Write-Host ('[FAIL] '+$s);exit $c}
$g=$null;if(Test-Path -LiteralPath (Join-Path $PackRoot 'Warhammer.exe') -PathType Leaf){$g=$PackRoot}else{$p=Split-Path -Parent $PackRoot;if(Test-Path -LiteralPath (Join-Path $p 'Warhammer.exe') -PathType Leaf){$g=$p}}
if(-not $g){F 'Warhammer.exe introuvable.' 2}
...
$state=(Get-Content -LiteralPath $l -TotalCount 1).Trim();$sp=Join-Path $state 'install_state.json';$m=Get-Content -LiteralPath $sp -Raw|ConvertFrom-Json;$g=[string]$m.game_root
...
Write-Host '[PASS] RC38 retire; runtime/config GitHub main exacts restaures.';exit 0
'''

UNINSTALL=r'''param([Parameter(Mandatory=$true)][string]$Root,[ValidateSet('Pre','Post')][string]$Phase='Post',[Parameter(Mandatory=$true)][string]$StateFile)
$ErrorActionPreference='Stop';$Root=[IO.Path]::GetFullPath($Root);$ov=Join-Path $Root 'BORDERLESS_AUTHORITY_RC38';$own=Join-Path $ov 'OVERLAY_OWNERSHIP.tsv'
function Sha([string]$p){return (Get-FileHash -LiteralPath $p -Algorithm SHA256).Hash.ToLowerInvariant()}
if($Phase -eq 'Pre'){
 $game='';$latest=Join-Path $Root '_PTAR_UNINSTALL\state\LATEST_STATE.txt'
 if(Test-Path -LiteralPath $latest -PathType Leaf){$st=(Get-Content -LiteralPath $latest -TotalCount 1).Trim();$js=Join-Path $st 'install_state.json';if(Test-Path -LiteralPath $js -PathType Leaf){$m=Get-Content -LiteralPath $js -Raw|ConvertFrom-Json;$game=[string]$m.game_root}}
 Set-Content -LiteralPath $StateFile -Value $game -Encoding UTF8;Write-Host ('[PASS] RC38 uninstall preflight game_root='+$game);exit 0
}
$game='';if(Test-Path -LiteralPath $StateFile -PathType Leaf){$game=(Get-Content -LiteralPath $StateFile -TotalCount 1).Trim()}
...
Write-Host '[PASS] Overlay RC38 retire par ownership/SHA.';exit 0
'''

def ps_scripts(vals:dict[str,str])->dict[str,str]:
    return {'install.ps1':_fill(INSTALL,vals),'verify.ps1':_fill(VERIFY,vals),'rollback.ps1':_fill(ROLLBACK,vals),'uninstall_overlay.ps1':_fill(UNINSTALL,vals)}
