...
int main(){
    ...
    if(FAILED(dev->CreateTexture2D(&pd,nullptr,&primary))||!primary)return 11;stage("02 PRIMARY\r\n");
    ...
    if(policy.map_texture2d(shared,nullptr,CallerDomain::Game).remapped)return 17;stage("05 POLICY\r\n");

    ID3D11Texture2D* familyRT=nullptr;ID3D11RenderTargetView* familyRTV=nullptr;stage("06 BEFORE_FAMILY_RT\r\n");
    if(FAILED(dev->CreateTexture2D(&probe,nullptr,&familyRT))||!dims(familyRT,1280,720)||!resource_has_family_tag(familyRT))return 18;stage("07 FAMILY_RT\r\n");
    if(FAILED(dev->CreateRenderTargetView(familyRT,nullptr,&familyRTV))||!familyRTV||!get_family_tag(familyRTV))return 19;stage("08 FAMILY_RTV\r\n");
    ctx->OMSetRenderTargets(1,&familyRTV,nullptr);
    D3D11_VIEWPORT v{300,150,600,300,0,1};ctx->RSSetViewports(1,&v);
    if(!vp_is(ctx,200,100,400,200))return 20;stage("09 FAMILY_VP\r\n");
    ...
    ID3D11Texture2D* familyDepth=nullptr;ID3D11DepthStencilView* familyDSV=nullptr;stage("10 BEFORE_DEPTH\r\n");
    if(FAILED(dev->CreateTexture2D(&dd,nullptr,&familyDepth))||!dims(familyDepth,1280,720)||!resource_has_family_tag(familyDepth))return 21;stage("11 DEPTH\r\n");
    if(FAILED(dev->CreateDepthStencilView(familyDepth,nullptr,&familyDSV))||!familyDSV||!get_family_tag(familyDSV))return 22;stage("12 DSV\r\n");
    ctx->ClearState();ctx->OMSetRenderTargets(0,nullptr,familyDSV);ctx->RSSetViewports(1,&v);
    if(!vp_is(ctx,200,100,400,200))return 23;stage("13 DSV_VP\r\n");

    ID3D11Texture2D* samePhysical=nullptr;ID3D11RenderTargetView* sameRTV=nullptr;D3D11_TEXTURE2D_DESC same=rt_desc(1280,720);stage("14 BEFORE_SAME\r\n");
    if(FAILED(dev->CreateTexture2D(&same,nullptr,&samePhysical))||!dims(samePhysical,1280,720)||resource_has_family_tag(samePhysical))return 24;stage("15 SAME\r\n");
    if(FAILED(dev->CreateRenderTargetView(samePhysical,nullptr,&sameRTV))||!sameRTV||get_family_tag(sameRTV))return 25;stage("16 SAME_RTV\r\n");
