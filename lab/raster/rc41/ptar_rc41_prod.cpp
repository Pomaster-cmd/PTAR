...
static bool runtime_layout_ok(HMODULE runtime,BYTE*& base) noexcept {
    ...
    // P1U46 primary swapchain vtable wrappers must already be installed before RC41 layers on top.
    IDXGISwapChain* sc=*reinterpret_cast<IDXGISwapChain**>(base+0x02C7E010u);
    if(!sc)return false;
    void** vt=*reinterpret_cast<void***>(sc);if(!vt)return false;
    if(vt[8]!=base+0x0000C6C0u||vt[9]!=base+0x0000CA10u||vt[12]!=base+0x0000CDB0u||vt[13]!=base+0x0000CE80u)return false;
    return true;
}
...
extern "C" __declspec(dllexport) int WINAPI PTAR_RC41_AutoStart(HMODULE runtime){
    ...
    if(!current_backbuffer(sc,&backbuffer,physicalW,physicalH)){log_line("FAIL RC41 physical backbuffer unavailable; fail-open");fail_cleanup();return -22;}
    const bool exact=(uint64_t(physicalW)*3u==uint64_t(outputW)*2u && uint64_t(physicalH)*3u==uint64_t(outputH)*2u);
    if(!exact){backbuffer->Release();log_dims("FAIL RC41 non-x1.5 geometry",physicalW,physicalH,outputW,outputH);fail_cleanup();return -23;}
    const Contract contract{{outputW,outputH},{physicalW,physicalH}};
    if(!g_contextHooks.configure(contract,backbuffer)){backbuffer->Release();log_line("FAIL RC41 context configure; fail-open");fail_cleanup();return -24;}
    backbuffer->Release();
    if(!g_contextHooks.install(ctx)){log_line("FAIL RC41 context hook install; fail-open");fail_cleanup();return -25;}

    HMODULE game=GetModuleHandleW(nullptr);
    if(!game||!g_self||!g_swapchainHooks.configure(contract,game,runtime,g_self,refresh_primary,nullptr)){
        log_line("FAIL RC41 swapchain configure; fail-open");fail_cleanup();return -26;
    }
    if(!g_swapchainHooks.install(sc)){log_line("FAIL RC41 swapchain hook install; fail-open");fail_cleanup();return -27;}

    InterlockedExchange(&g_active,1);InterlockedExchange(&g_starting,0);
    log_dims("ACTIVE_RC41_LOGICAL_NATIVE_SUBRASTER logical/physical",outputW,outputH,physicalW,physicalH);
    return 0;
}
