...
b64 = b"".join(b"".join((STAGE/n).read_bytes().split()) for n in CHUNKS)
...
with tempfile.TemporaryDirectory(prefix="ptar-s11-") as td:
    ...
    tar_path = Path(td)/"payload.tar"; tar_path.write_bytes(tar_bytes)
    with tarfile.open(tar_path, "r:") as tf: safe_extract(tf, out)
    fused_delta_p = out/"diag/base/FUSEDDETAIL1_FROM_GW12.xor.xz"
    runtime_delta_p = out/"diag/RUNTIME_FROM_GW12.xor.xz"
    fd = lzma.decompress(fused_delta_p.read_bytes()); rd = lzma.decompress(runtime_delta_p.read_bytes())
    if len(fd) != len(gw12) or len(rd) != len(gw12): die("delta length mismatch")
    fused = bytes(a^b for a,b in zip(gw12,fd)); runtime = bytes(a^b for a,b in zip(gw12,rd))
    ...
    fused_delta_p.unlink(); runtime_delta_p.unlink()
    ...
    actual = {p.relative_to(out).as_posix(): sha256(p.read_bytes()) for p in sorted(out.rglob("*")) if p.is_file()}
    if set(actual) != set(expected): die("path set mismatch")
...
if gitblob((ROOT/"payload/d3d11.dll").read_bytes()) != "91a9cbb8001757cad55220ea93153f44984430b9": die("runtime Git blob mismatch")
