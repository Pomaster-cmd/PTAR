...
public:
    ...
    {
        ComPtr<ID3DBlob> v,m,k;

        std::cout << "[HLSL] Compiling VS / vs_5_0" << std::endl;
        HRESULT hr=CompileHlslFile(vsPath,"main","vs_5_0",&v);
        if(FAILED(hr)) return hr;

        std::cout << "[HLSL] Compiling PTAR-NG MoE v01 SF5 / ps_5_0" << std::endl;
        hr=CompileHlslFile(moePath,"main","ps_5_0",&m);
        if(FAILED(hr)) return hr;

        std::cout << "[HLSL] Compiling EDGE-NG v03 K185 control / ps_5_0" << std::endl;
        hr=CompileHlslFile(k185Path,"main","ps_5_0",&k);
        if(FAILED(hr)) return hr;

        hr=WriteBlobFile(JoinPath(artifactDir,L"ptar_vs.cso"),v.Get());
        if(FAILED(hr)) return hr;
        hr=WriteBlobFile(JoinPath(artifactDir,L"ptar_moe_ng_v01_sf5_ps.cso"),m.Get());
        if(FAILED(hr)) return hr;
        hr=WriteBlobFile(JoinPath(artifactDir,L"ptar_k185_control_ps.cso"),k.Get());
        if(FAILED(hr)) return hr;

        std::cout << "[DXBC] Auditing complete MoE shared-fetch shader" << std::endl;
        hr=AuditK185Bytecode(m.Get(),JoinPath(artifactDir,L"ptar_moe_ng_v01_sf5_ps.asm"));
        if(FAILED(hr)) return hr;

        std::cout << "[DXBC] Auditing K185 control shader" << std::endl;
        hr=AuditK185Bytecode(k.Get(),JoinPath(artifactDir,L"ptar_k185_control_ps.asm"));
        if(FAILED(hr)) return hr;

        hr=m_device->CreateVertexShader(v->GetBufferPointer(),v->GetBufferSize(),0,&m_vs);
        if(FAILED(hr)) return hr;
        hr=m_device->CreatePixelShader(m->GetBufferPointer(),m->GetBufferSize(),0,&m_psMoe);
        if(FAILED(hr)) return hr;
        return m_device->CreatePixelShader(k->GetBufferPointer(),k->GetBufferSize(),0,&m_psK185);
    }
...
static int Usage()
{
    std::wcout << L"Usage: ptar_moe_ng_v01_hw_validation.exe --root <PTAR_PROJECT_MASTER> "
                  L"--vs-src <vs.hlsl> --moe-src <moe.hlsl> --k185-src <k185.hlsl> --out <results> [--allow-any-adapter]"
               << std::endl;
    return 2;
}
...
{
    std::wstring root,vsPath,moePath,k185Path,outDir;
    bool allowAny=false;
    for(int i=1;i<argc;++i)
    {
        ...
        else if(a==L"--moe-src" && i+1<argc) moePath=argv[++i];
        else if(a==L"--k185-src" && i+1<argc) k185Path=argv[++i];
        else if(a==L"--out" && i+1<argc) outDir=argv[++i];
        else if(a==L"--allow-any-adapter") allowAny=true;
        else return Usage();
    }
    if(root.empty()||vsPath.empty()||moePath.empty()||k185Path.empty()||outDir.empty()) return Usage();
    ...
    hr=gpu.CompileAndLoadShaders(vsPath,moePath,k185Path,outDir);
    ...
    const std::wstring expectedDir=JoinPath(root,L"benchmarks\\results\\current\\MOE_NG_V01_SF5\\MOE_SEMANTIC_FLOAT32_OUTPUTS");
    ...
    if(summary)
    {
        ...
        fprintf(summary,"PTAR-NG MoE v01 SF5 HARDWARE VALIDATION\r\n");
