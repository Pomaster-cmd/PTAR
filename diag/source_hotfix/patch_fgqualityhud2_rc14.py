...
TARGETS={
'RC14_QUALITY_CALC_WIDTH':0x34FD283,
'RC14_QUALITY_HOTKEY_ACTION0':0x34FD2EF,
'RC14_QUALITY_STATUS':0x34FD3CF,
'QUALITY_PROFILE':0x34FC09C,
'QUALITY_APPLIED_TIER':0x34FC0A4,
'QUALITY_PENDING_ME':0x34FC0A8,
'QUALITY_ACTIVE_PROFILE':ACTIVE_PROFILE_RVA,
'FG_SCHEDULER_RUNNING':0x2C93B94,
'NOTICE_FUNC':0x16270,
'LOG_FUNC':0x1E50,
}
REQ=('quality_calc_width_hud2','quality_hotkey_hud2','quality_status_hud2')
HOOKS={0x220BD:(0x34FD283,'quality_calc_width_hud2',15),0x12225:(0x34FD2EF,'quality_hotkey_hud2',7),0x12271:(0x34FD3CF,'quality_status_hud2',5)}
PATCHES={
...
def main(base,obj,out):
 ...
 # Critical field-regression guard: RC14 startup quality parser must remain exact.
 if ct(src,pi,0x2B46)!=0x34FD200:raise RuntimeError('RC14 parser hook unexpected')
 parser_bytes=bytes(src[roff(pi,0x2B46):roff(pi,0x2B46)+22])
 ...
 # Assert startup parser is byte-identical to field-good RC14.
 if bytes(dst[roff(pi,0x2B46):roff(pi,0x2B46)+22])!=parser_bytes:raise RuntimeError('startup parser changed')
 ...
 print('OUTPUT_SHA256='+sha(dst));print('PE_CHECKSUM=0x%X'%cs);print('HUD_CODE_RVA=0x%X'%NEW_CODE_RVA);print('HUD_CODE_SIZE=0x%X'%len(code));print('RC14_STARTUP_PARSER_PRESERVED=YES')
 for n in REQ: print('%s_RVA=0x%X'%(n.upper(),NEW_CODE_RVA+sy[n]))
if __name__=='__main__':
 if len(sys.argv)!=4:raise SystemExit('usage: patch_fgqualityhud2_rc14.py RC14_DLL OBJ OUT_DLL')
 main(*sys.argv[1:])
