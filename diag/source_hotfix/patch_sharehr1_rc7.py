...
BASE_SHA='f871c6a5e3a6a7cf1430d5fdd609d5e5f81a98f5c141559af81a5d51718cd2d2'
...
TARGETS={'SUCCESS_TARGET':SUCCESS_RVA,'FAILURE_TARGET':FAILURE_RVA,'LAST_STATUS':LAST_STATUS_RVA}
...
def parse_pe(b):
 ...
 return dict(e=e,coff=coff,opt=opt,sh=sh,n=nsec,secs=secs,ib=struct.unpack_from('<Q',b,opt+24)[0],cks_off=opt+64)
...
def patch(base,obj,outp):
 ...
 if sha(src)!=BASE_SHA: raise RuntimeError('wrong RC7 base hash '+sha(src))
 pe=parse_pe(src)
 ...
 if seen!=set(TARGETS): raise RuntimeError('missing relocs '+repr(set(TARGETS)-seen))
 if len(code)>d['vs']: raise RuntimeError('new code exceeds existing diagnostic virtual size')
 out=bytearray(src)
 # Replace executable bytes only; clear the rest of the old SHAREDIAG1 virtual body with INT3.
 out[d['rp']:d['rp']+d['vs']]=b'\xCC'*d['vs']
 ...
 print('SHAREHR1_CODE_SIZE=0x%X'%len(code))
 print('PE_CHECKSUM=0x%X'%c)

if __name__=='__main__':
 if len(sys.argv)!=4: raise SystemExit('usage: patch_sharehr1_rc7.py RC7_DLL OBJ OUT_DLL')
 patch(*sys.argv[1:])
