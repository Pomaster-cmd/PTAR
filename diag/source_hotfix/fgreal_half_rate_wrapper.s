...
FGRealGovernorWrapper:
    ...
    # This wrapper is reached only from the already-gated active-FG branches
    # of HookPresent/HookPresent1. FG OFF never executes this code.
...
.wait_loop:
    mov rax, r9
    sub rax, r8
    xor ecx, ecx                 # Sleep(0) for the final short phase
    cmp rax, qword ptr [rip + GOV_SLEEP1_TICKS]
    jbe .do_sleep
    mov ecx, 1                   # Sleep(1) while > ~2.5 ms remain
.do_sleep:
...
.late_rebase:
    # Never burst to catch up after a slow source frame. Submit the late REAL
    # immediately and restart the half-rate phase from the actual current time.
    mov rax, r8
    add rax, qword ptr [rip + GOV_INTERVAL]
    mov qword ptr [rip + GOV_NEXT_TICK], rax
    jmp .submit
...
.initialize:
    ...
    # REAL interval = 2 / FrameGenerationTargetFPS seconds.
    # For target 60 this is 1/30 s.
    shl rax, 1
    xor edx, edx
    mov r10d, dword ptr [rip + FG_TARGET_FPS]
    mov r11d, r10d
    ...
    # Mirror the existing validated QPC scheduler's coarse/fine handoff:
    # Sleep(1) above ~2.5 ms, Sleep(0) below it.
    mov rax, qword ptr [rsp + 56]
    xor edx, edx
    mov r11d, 400
    div r11
...
.have_sleep_threshold:
    ...
    # First REAL after activation/target change is immediate; subsequent REALs
    # are phase-locked to the half-rate cadence.
    lea rcx, [rsp + 48]
    call qword ptr [rip + QPC_IAT]
    test eax, eax
    je .fail_open
