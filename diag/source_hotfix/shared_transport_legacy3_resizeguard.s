...
.extern FAKE_VTABLE
.extern FAKE_OBJECT
.extern LEGACY_ACTIVE
.extern FG_SCHEDULER_RUNNING
.extern RESIZE_RETURN_EPILOGUE
...
mailbox_call_wrapper:
    # Persist which transport mode actually reached the mailbox stage. Keyed
    # primary writes 0; Inquisitor legacy fallback writes 1. This runtime mode
    # is used only by RESIZEGUARD1 and does not alter keyed SatGat behavior.
    mov r10d, dword ptr [rip + FALLBACK_FLAG]
    mov dword ptr [rip + LEGACY_ACTIVE], r10d
    sub rsp, 0x28
    call MAILBOX_FUNC
    add rsp, 0x28
    mov dword ptr [rip + FALLBACK_FLAG], 0
...
# RESIZEGUARD1: only the legacy-active FG path locks ResizeBuffers.
# This specifically prevents Inquisitor's continuously failing native-size
# ResizeBuffers requests from tearing down a just-started legacy FG pipeline.
# Keyed-primary sessions (SatGat) keep LEGACY_ACTIVE=0 and pass through.
ResizeBuffersFGGuard:
    cmp dword ptr [rip + LEGACY_ACTIVE], 0
    je .rg_passthrough
    cmp dword ptr [rip + FG_SCHEDULER_RUNNING], 0
    je .rg_passthrough
    mov ebp, 0x887A0001              # DXGI_ERROR_INVALID_CALL
    add rsp, 8                       # discard helper CALL return address
    jmp RESIZE_RETURN_EPILOGUE
.rg_passthrough:
    mov ebp, edx
    mov r15d, dword ptr [rsp + 0x150]
    ret
