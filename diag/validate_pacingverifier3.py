...
ck('runtime exact GW16',sha(ROOT/'payload/d3d11.dll')=='864c0ca8f24f22f6a3cd4c21a0e213f431f5268e69b04e3860c52fe72600fc3c',sha(ROOT/'payload/d3d11.dll'))
...
ck('observer stall threshold present','capDt > 1.75 * refreshMs' in src)
ck('observer recovery quarantines two transitions','observerRecoveryTransitions = 2' in src and 'observerRecoveryTransitions--' in src)
ck('observer misses separated from PTAR gaps',all(x in src for x in ['OBSERVER MISSED CONTENTS','observerMissedContents += delta - 1','transitionContaminated']))
ck('observer-contaminated pacing excluded','frames[i].TransitionContaminated' in src and 'observerExcludedPacingIntervals++' in src)
ck('observer clean coverage gates verdict','PACING CLEAN COVERAGE PCT' in src and 'cleanPacingCoverage >= 85.0' in src)
ck('observer classifier target self-test','ObserverClassificationSelfTest()' in src and 'SELFTEST=FAIL OBSERVER_CLASSIFICATION' in src)
...
# Real GW15 observer-stall regression: verifier paused 89.746ms, then observed
# serial 1067 R -> 1069 R -> 1070 G. New logic must classify the +2/same-type
# transition as observer loss and quarantine the next transition too.
recovery=2; obs_miss=0; true_gap=0; true_same=0; contaminated=0
seq=[(1067,False),(1069,False),(1070,True)]
for i in range(1,len(seq)):
    d=(seq[i][0]-seq[i-1][0])&0xFFF
    if recovery>0:
        contaminated+=1
        if 1<=d<=2048 and d>1: obs_miss+=d-1
        recovery-=1
    else:
        if 1<=d<=2048 and d>1: true_gap+=d-1
        if seq[i][1]==seq[i-1][1]: true_same+=1
ck('GW15 89.746ms observer fixture => one observer miss',obs_miss==1,obs_miss)
ck('GW15 observer fixture => no PTAR gap',true_gap==0,true_gap)
ck('GW15 observer fixture => no PTAR same-type break',true_same==0,true_same)
ck('GW15 observer fixture quarantines two pacing transitions',contaminated==2,contaminated)
...
print('PACINGVERIFIER3_VALIDATION=%d/%d PASS'%(passed,len(checks)))
