...
EXPECTED='bc291f0f91013df7a28630ffef44983856fce6eb71d79aca597ab292012165e0'
HUDREC_INTERMEDIATE='e81e4c6239462bc7a93c3fd7d7abb4bd96e09db1f013eb48a46f40341ffa6429'
...
ck('runtime exact GW16I retaining HUDREC1',sha(DLL)==EXPECTED,sha(DLL))
ck('mirror exact GW16I retaining HUDREC1',sha(MIRROR)==EXPECTED,sha(MIRROR))
ck('runtime mirrors exact',a==b)
ck('GW16I runtime size expected',len(a)==321024,len(a))
...
if BASEFILE.is_file():
    ...
    ck('GW16I input extension is exact 1024-byte append',len(a)==len(base_bytes)+1024,len(a)-len(base_bytes))
...
with tempfile.TemporaryDirectory(prefix='ptar_hudrec1_') as td:
    ...
    ck('HUDREC1 patcher reproduces inherited HUDREC1 intermediate',rebuilt.is_file() and sha(rebuilt)==HUDREC_INTERMEDIATE,sha(rebuilt) if rebuilt.is_file() else 'missing')
