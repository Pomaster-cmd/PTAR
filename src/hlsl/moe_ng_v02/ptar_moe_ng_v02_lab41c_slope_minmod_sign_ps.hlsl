// PTAR-NG MoE v02 - LAB41C minmod-sign slope codegen probe
...
float4 MCSlope2(float4 a,float4 b){float4 sum=a+b;float4 lim=4.0f*min(abs(a),abs(b));float4 mag=min(abs(sum),lim);return 0.5f*(sign(a)+sign(b))*mag;}
