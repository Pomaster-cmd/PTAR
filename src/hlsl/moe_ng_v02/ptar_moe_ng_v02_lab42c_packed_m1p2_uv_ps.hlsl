// PTAR-NG MoE v02 - LAB42C packed -axis/+2axis UV candidate
...
float4 main(PSIn input):SV_Target{
 ...
 float4 uvM1P2=mad(axisUV.xyxy,float4(-1.0f,-1.0f,2.0f,2.0f),baseUV.xyxy);
 float2 uvP1=baseUV+axisUV;
 float4 fm1=gSource.SampleLevel(gLinearClamp,uvM1P2.xy,0.0f),f0=gSource.SampleLevel(gLinearClamp,baseUV,0.0f);
 float phaseFrac=frac(useX?srcPos.x:srcPos.y);
 float4 f1=gSource.SampleLevel(gLinearClamp,uvP1,0.0f),f2=gSource.SampleLevel(gLinearClamp,uvM1P2.zw,0.0f);
