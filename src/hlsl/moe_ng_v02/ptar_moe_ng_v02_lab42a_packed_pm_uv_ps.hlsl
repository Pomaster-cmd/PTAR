// PTAR-NG MoE v02 - LAB42A packed -axis/+axis UV candidate
// Baseline: replicated hardware-selected LAB38A. Exact x1.5 only. ABI unchanged.
// Computes the -axis and +axis sample coordinates together in one float4 ADD.
...
float4 main(PSIn input):SV_Target{
 ...
 float4 uvMP=baseUV.xyxy+float4(-axisUV,axisUV);
 float2 uvP2=mad(axisUV,2.0f,baseUV);
 float4 fm1=gSource.SampleLevel(gLinearClamp,uvMP.xy,0.0f),f0=gSource.SampleLevel(gLinearClamp,baseUV,0.0f);
 float phaseFrac=frac(useX?srcPos.x:srcPos.y);
 float4 f1=gSource.SampleLevel(gLinearClamp,uvMP.zw,0.0f),f2=gSource.SampleLevel(gLinearClamp,uvP2,0.0f);
