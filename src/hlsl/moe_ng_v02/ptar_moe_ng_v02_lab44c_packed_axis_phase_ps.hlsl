// PTAR-NG MoE v02 - LAB44C packed axis/phase select
// Baseline: replicated hardware-selected LAB38A. Exact x1.5 only. ABI unchanged.
// Packs axisUV.xy and the phase source into one direction-dependent float4 select.
...
float4 main(PSIn input):SV_Target{
 ...
 float4 axisPhase=useX?float4(gInvInputSize.x,0.0f,srcPos.x,0.0f):float4(0.0f,gInvInputSize.y,srcPos.y,0.0f);
 float2 axisUV=axisPhase.xy;
 float4 fm1=gSource.SampleLevel(gLinearClamp,baseUV-axisUV,0.0f),f0=gSource.SampleLevel(gLinearClamp,baseUV,0.0f);
 float phaseFrac=frac(axisPhase.z);
