// PTAR-NG MoE v02 - LAB24 Host-Supplied Inverse Input Size
// Direct3D 11 / Shader Model 5.0 / Windows 8.1 target.
//
// Starting point: hardware-validated LAB22 phase-fused MC.
// Goal: eliminate all texture-coordinate divisions from the pixel shader.
//
// LAB ABI for this candidate only:
//   cb0.xy = input size (retained for compatibility/diagnostics)
//   cb0.zw = reciprocal input size supplied by the host
//
// The historical gOutputSize field is unused by the MoE shader, so LAB24
// repurposes those two floats in the experimental runtime ABI. Admission
// requires WARP equivalence and independent CPU parity before any hardware run.
//
...
{
    return (texelPos+0.5f)*gInvInputSize;
}
