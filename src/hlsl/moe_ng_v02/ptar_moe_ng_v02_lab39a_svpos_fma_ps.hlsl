// PTAR-NG MoE v02 - LAB39A SV_Position fused exact x1.5 mapping candidate
// Baseline: LAB38A. Exact x1.5 only. ABI and reconstruction unchanged.
// Uses the D3D11 pixel-center invariant SV_Position.xy = integer pixel + 0.5
// to synthesize srcPos directly with MAD: pos*(2/3) - 1/3.
// Intended codegen change: ROUND_NI + MUL -> one MAD, with bit-exact output required.
...
float4 main(PSIn input):SV_Target{
 float2 srcPos=mad(input.position.xy,2.0f/3.0f,-1.0f/3.0f),srcFloor=floor(srcPos);
