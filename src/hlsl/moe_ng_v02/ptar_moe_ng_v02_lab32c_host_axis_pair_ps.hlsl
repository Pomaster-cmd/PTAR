// PTAR-NG MoE v02 - LAB32C Host Axis Pair
// Baseline: hardware-selected LAB31G. Exact x1.5 only.
// Experimental 32-byte ABI:
// cb0.xy = inverse input size; cb0.zw = half inverse input size;
// cb1.xy = (invX,0); cb1.zw = (0,invY).
Texture2D<float4> gSource:register(t0);SamplerState gLinearClamp:register(s0);
cbuffer PTARMoeNGV02Constants:register(b0){float2 gInvInputSize;float2 gHalfInvInputSize;float4 gAxisUVPair;};
...
float4 main(PSIn input):SV_Target{
 ...
 float2 axisUV=useX?gAxisUVPair.xy:gAxisUVPair.zw;
