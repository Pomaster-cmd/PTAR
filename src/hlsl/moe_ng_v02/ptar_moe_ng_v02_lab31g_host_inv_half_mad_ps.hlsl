// PTAR-NG MoE v02 - LAB31G Host Inverse + Half-Inverse MAD
// Baseline: hardware-selected LAB30C. Exact x1.5 only.
// Experimental ABI: cb0.xy = inverse input size, cb0.zw = half inverse input size.
// Host supplies both vectors so gather/base center mappings require no DIV and
// can each be expressed as a single MAD. Hardware promotion requires a host ABI
// implementation and full software equivalence first.
...
float4 main(PSIn input) : SV_Target
{
    ...
    float2 gatherUV=mad(srcFloor,gInvInputSize,gInvInputSize);
    ...
    float2 baseUV=mad(basePos,gInvInputSize,gHalfInvInputSize);
