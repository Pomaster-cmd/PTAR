// PTAR-NG MoE v02 - LAB44B packed base/phase/inverse-axis select
// Baseline: replicated hardware-selected LAB38A. Exact x1.5 only. ABI unchanged.
// Packs basePos.xy, phase source and selected inverse-axis scalar into one float4 select.
...
float4 main(PSIn input):SV_Target{
 ...
 float4 geom=useX?float4(srcFloor.x,srcPos.y,srcPos.x,gInvInputSize.x):float4(srcPos.x,srcFloor.y,srcPos.y,gInvInputSize.y);
 float2 baseUV=mad(geom.xy,gInvInputSize,gHalfInvInputSize);
 float2 axisUV=float2(useX?geom.w:0.0f,useX?0.0f:geom.w);
 float4 fm1=gSource.SampleLevel(gLinearClamp,baseUV-axisUV,0.0f),f0=gSource.SampleLevel(gLinearClamp,baseUV,0.0f);
 float phaseFrac=frac(geom.z);
