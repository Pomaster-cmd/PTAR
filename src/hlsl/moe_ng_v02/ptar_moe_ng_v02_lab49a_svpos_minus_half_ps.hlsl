// PTAR-NG MoE v02 - LAB49A SV_Position center subtraction candidate
// Research baseline: replicated hardware-selected LAB47A, itself derived from LAB38A.
// Exact x1.5 only. Algorithm and ABI unchanged.
// Replaces floor(SV_Position.xy) with SV_Position.xy - 0.5, relying only on the
// D3D11 pixel-center convention. Full WARP bit-exactness is mandatory before use.
...
float4 main(PSIn input):SV_Target{
 float2 outPix=input.position.xy-0.5f,srcPos=outPix*(2.0f/3.0f),srcFloor=floor(srcPos);
