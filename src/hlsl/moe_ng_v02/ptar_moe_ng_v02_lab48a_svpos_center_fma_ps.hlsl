// PTAR-NG MoE v02 - LAB48A SV_Position center-FMA candidate
// Research baseline: replicated hardware-selected LAB47A.
// Fixed fullscreen raster contract: SV_Position.xy is the D3D11 pixel center (n + 0.5).
// Therefore srcPos = floor(SV_Position.xy)*(2/3) can be formed as one MAD:
// mad(SV_Position.xy, 2/3, -1/3). The later LAB47 phase-zero DXBC peephole is
// applied separately after FXC codegen. Texture schedule and 16-byte ABI are unchanged.
