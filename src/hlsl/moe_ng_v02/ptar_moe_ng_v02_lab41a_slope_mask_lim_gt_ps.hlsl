// PTAR-NG MoE v02 - LAB41A slope limiter-mask (>0) codegen probe
// Baseline: replicated hardware-selected LAB38A. Exact x1.5 only. ABI unchanged.
// Moves the same-sign selection from the final slope value to the limiter.
...
float4 MCSlope2(float4 a,float4 b){float4 sum=a+b;float4 lim=4.0f*min(abs(a),abs(b));lim=(a*b>0.0f)?lim:0.0f;return clamp(sum,-lim,lim);}
