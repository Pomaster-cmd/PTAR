// PTAR-NG MoE v02 - LAB33A SV_Position direct MAD mapping
// Baseline: hardware-selected LAB31G. Exact x1.5 only. ABI unchanged.
// D3D11 pixel-center SV_Position.xy is n+0.5, so source position can be formed
// directly as mad(SV_Position.xy, 2/3, -1/3) instead of floor(SV_Position.xy)*2/3.
...
float4 main(PSIn input):SV_Target{
 float2 srcPos=mad(input.position.xy,float2(2.0f/3.0f,2.0f/3.0f),float2(-1.0f/3.0f,-1.0f/3.0f));
 float2 srcFloor=floor(srcPos);
 float4 g=gSource.GatherGreen(gLinearClamp,mad(srcFloor,gInvInputSize,gInvInputSize));float tl=g.w,tr=g.z,bl=g.x,br=g.y;
 float gx=(tr+br)-(tl+bl),gy=(bl+br)-(tl+tr);bool useX=abs(gx)>=abs(gy);
