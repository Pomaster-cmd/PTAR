// PTAR-NG MoE v02 - LAB31B Reciprocal Hoist + Direct Axis UV
// Baseline: hardware-selected LAB30C. Exact x1.5 only.
// Goal: replace three vector divisions by one reciprocal computation reused for
// all texture coordinates, and materialize axisUV directly from that reciprocal.
// ABI unchanged. Admission requires bit-exact WARP parity before hardware.
...
float4 main(PSIn input) : SV_Target
{
    ...
    float2 baseUV=(basePos+0.5f)*invInputSize;
    float2 axisUV=useX?float2(invInputSize.x,0.0f):float2(0.0f,invInputSize.y);
