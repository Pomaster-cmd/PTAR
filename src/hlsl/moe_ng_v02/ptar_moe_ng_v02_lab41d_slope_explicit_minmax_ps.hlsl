// PTAR-NG MoE v02 - LAB41D explicit min/max slope codegen probe
...
float4 MCSlope2(float4 a,float4 b){float4 sum=a+b;float4 lim=4.0f*min(abs(a),abs(b));float4 limited=min(max(sum,-lim),lim);return (a*b>=0.0f)?limited:0.0f;}
