// PTAR-NG MoE v02 - LAB44A packed base/axis geometry select
// Baseline: replicated hardware-selected LAB38A. Exact x1.5 only. ABI unchanged.
// Packs basePos.xy and axisUV.xy into one direction-dependent float4 select.
...
float4 main(PSIn input):SV_Target{
 ...
 float4 geom=useX?float4(srcFloor.x,srcPos.y,gInvInputSize.x,0.0f):float4(srcPos.x,srcFloor.y,0.0f,gInvInputSize.y);
 float2 baseUV=mad(geom.xy,gInvInputSize,gHalfInvInputSize);float2 axisUV=geom.zw;
