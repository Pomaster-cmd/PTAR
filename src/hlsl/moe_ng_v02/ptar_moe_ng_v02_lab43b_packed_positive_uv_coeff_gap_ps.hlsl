// PTAR-NG MoE v02 - LAB43B packed positive UV with coefficient gap
// Baseline: hardware-selected LAB38A. Exact x1.5 only. ABI unchanged.
// Preserve LAB38A's first two texture issues, pack only +axis/+2axis UVs,
// and compute rounded phase coefficients before issuing the final samples.
...
float4 main(PSIn input):SV_Target{
 ...
 float4 plusUV=mad(axisUV.xyxy,float4(1.0f,1.0f,2.0f,2.0f),baseUV.xyxy);
 float phaseHigh=round(phaseFrac);
 float4 coeff=mad(phaseHigh,float4(-13.0f,-1.0f,13.0f,-1.0f),float4(20.0f,2.0f,7.0f,-1.0f));
 float4 f1=gSource.SampleLevel(gLinearClamp,plusUV.xy,0.0f),f2=gSource.SampleLevel(gLinearClamp,plusUV.zw,0.0f);
