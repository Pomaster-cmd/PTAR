// PTAR-NG MoE v02 - LAB33D direct SV_Position mapping + diagonal-product direction test
// Baseline: hardware-selected LAB31G. Exact x1.5 only. ABI unchanged.
// Direction identity: |gx|>=|gy| iff (br-tl)*(tr-bl)>=0 for finite samples.
...
float4 main(PSIn input):SV_Target{
 float2 srcPos=mad(input.position.xy,float2(2.0f/3.0f,2.0f/3.0f),float2(-1.0f/3.0f,-1.0f/3.0f)),srcFloor=floor(srcPos);
