// PTAR-NG MoE v02 - LAB40A exact scheduling/register-pressure candidate
// Baseline: replicated hardware-selected LAB38A. Exact x1.5 only. ABI unchanged.
// Moves phase coefficient generation earlier and consumes m0x2 into the cubic accumulator
// before building m1x2. Arithmetic dependency/order of the final cubic remains LAB38A-equivalent.
...
float4 main(PSIn input):SV_Target{
 ...
 float4 d0=f0-fm1,d1=f1-f0;
 float4 m0x2=MCSlope2(d0,d1);
 float4 acc=coeff.y*m0x2;
 acc=mad(coeff.x,f0,acc);
 acc=mad(coeff.z,f1,acc);
 float4 d2=f2-f1,m1x2=MCSlope2(d1,d2);
 acc=mad(coeff.w,m1x2,acc);
 float4 cubic=acc*(1.0f/27.0f);
